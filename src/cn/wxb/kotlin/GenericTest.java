 ...
 *  16。cpu 缓存一致性协议 如intel的 MESI协议 - 保证了每个缓存中使用的共享变量的副本是一致的
 *      当CPU写数据时，如果发现操作的变量是共享变量，即在其他CPU中也存在该变量的副本，会发出信号通知其他CPU将该变量的缓存行置为无效状态，
 *      因此当其他CPU需要读取这个变量时，发现自己缓存中缓存该变量的缓存行是无效的，那么它就会从内存重新读取。
 *  17。cpu乱序执行优化 - 为了使得处理器内部的运算单元能够尽量被充分利用 （JVM也有类似指令重排优化）
 *      处理器会在计算之后将乱序执行的结果重组，保证该结果与顺序执行是一致的，但不保证程序中各个语句执行的先后顺序和输入的顺序一致
 *
 */
